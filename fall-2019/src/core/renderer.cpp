...
#endif
			Sampler sampler = Sampler(260746982 + y);
			for (int x = 0; x < scene.config.width; ++x) {
				float redColorSum = 0.0;
				float greenColorSum = 0.0;
				float blueColorSum = 0.0;
				float sample1 = 0.0f;
				float sample2 = 0.0f;
				...
				for (int i = 0; i < rayAmnt; i++) {
					...
					redColorSum += pixelColour.x;
					greenColorSum += pixelColour.y;
					blueColorSum += pixelColour.z;

				}
				v3f finalPixelColour = v3f(redColorSum / (float)rayAmnt, greenColorSum / (float)rayAmnt, blueColorSum / (float)rayAmnt);
				integrator->rgb->data[x + scene.config.width * y] = finalPixelColour;
