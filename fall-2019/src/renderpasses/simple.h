...
struct SimplePass : RenderPass {
    ...
    virtual void render() override {
        ...
        for (size_t i = 0; i < objects.size(); i++) {
			...
			GLuint lightIntUniform = GLuint(glGetUniformLocation(obj.shaderID, "lightInt"));
			glUniform3f(lightIntUniform, this->lightIntensity.x, this->lightIntensity.y, this->lightIntensity.z);
